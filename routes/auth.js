...
router.post("/tempuser", async (req, res) => {
  try {
    ...
    const msg = {
      from: "Twitter Doom <identity@pluto.uz>",
      to: email,
      subject: `Verification code - ${verificationCode}`,
      templateId: "d-4a898690e4dd4ce4aced33f953a11b73",
      dynamicTemplateData: {
    ...
    res
      .status(201)
      .json({ message: "Temprorary user created and verification code is sent", status: 201 });
  } catch (e) {
    console.log(e);
    return res.status(500).json({ message: "Something went wrong. Try again." });
  }
...
router.post(
  ...
  [
    ...
    check(
      "verificationCode",
      "Verification code cannot be empty, less than 6 characters and should be numeric"
    )
      .notEmpty()
      .isLength({ min: 6 }),
  ],
  async (req, res) => {
    try {
      ...
      if (!tempUserCandidate) {
        return res.status(400).json({
          message: "User with email was not granted a verification code before. Please try again",
        });
      }
